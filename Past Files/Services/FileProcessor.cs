...
using FileTrackerApp.Data;
using FileTrackerApp.Models;
using FileTrackerApp.Utils;
using Microsoft.EntityFrameworkCore;
using Serilog;

namespace FileTrackerApp.Services
{
    public class FileProcessor
    {
        ...
        private void LoadRecordsIntoMemory()
        {
            try
            {
                ...
                _identityMap = identities.ToDictionary(
                    i => (i.VolumeSerialNumber, i.FileID),
                    i => i
                );
        ...
        public void ProcessFile(string filePath)
        {
            try
            {
                ...
                (var fileID, var volumeSerialNumber) = FileIdentifier.GetFileID(filePath);
                var fileIdentityKey = (VolumeSerialNumber: volumeSerialNumber, FileID: fileID);
        ...
        public void GetFileInformation(string fileName)
        {
            try
            {
                ...
                foreach (var file in files)
                {
                    ...
                    {
                        Console.WriteLine($"\tVolume Serial Number: {identity.VolumeSerialNumber}, File ID: {identity.FileID}");
                    }
