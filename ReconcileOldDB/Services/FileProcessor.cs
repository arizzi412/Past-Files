...
public class FileProcessor
{
    ...
    private readonly FileTrackerContext _oldDBContext;

    private Dictionary<(uint VolumeSerialNumber, ulong FileID), FileIdentity> _identityMap = [];
    private Dictionary<(uint VolumeSerialNumber, ulong FileID), FileIdentity> _oldDBidentityMap = [];
    private Dictionary<string, FileRecord> _hashMap = [];
    private Dictionary<string, FileRecord> _hashMapOldDB = [];


    public FileProcessor()
    {
        _context = new FileTrackerContext();
        _oldDBContext = new FileTrackerContext("filetracker - Copy.db");
    ...
    private void LoadRecordsIntoMemory()
    {
        try
        {
            ...
            var fileRecords = _context.FileRecords
                ...
                .AsNoTracking()
               // .Where(f => !string.IsNullOrEmpty(f.Hash)) // I dont think we need this
                .ToList();
    ...
    private void LoadOldDBRecordsIntoMemory()
    {
        try
        {
            // Load FileIdentities into a dictionary
            var identities = _oldDBContext.FileIdentities
                .Include(i => i.FileRecord)
                .AsNoTracking()
                .ToList();

            _oldDBidentityMap = identities.ToDictionary(
                i => (i.VolumeSerialNumber, i.NTFSFileID),
                i => i
            );
            ...
            _hashMapOldDB = fileRecords.ToDictionary(f => f.Hash, f => f);
        }
        catch (Exception ex)
        {
            Console.WriteLine($"Error loading records into memory: {ex.Message}");
    ...
    public void ProcessFile(string filePath)
    {
        try
        {
            ...
            if (fileRecord == null)
            {
                fileHash = GetHashIncludingFromOldDB(filePath, fileIdentityKey);
    ...
    private string GetHashIncludingFromOldDB(string filePath, (uint VolumeSerialNumber, ulong FileID) fileIdentityKey )
    {
        if (_oldDBidentityMap.TryGetValue(fileIdentityKey, out var identityOldDb))
        {
            var fileRecord = identityOldDb.FileRecord;
